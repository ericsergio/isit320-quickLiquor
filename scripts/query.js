...
function placeControls(that) {
    ...
    if ($("button").length < 2) {
        ...
        $(".results").before(
            '<button id = "sendOrder" onclick = "apiQuery()">Send Order</button>'
        );
    }
}
...
function doOrder() {
    for (p in Items) {
        let url = "http://liquorrus.hopto.org:3000/getitems/";
        url = url + Items[p];
        urlItems.push(url);
    }
}
...
const doTempTbl = async () => {
    await $.get("http://liquorrus.hopto.org:3000/createordertable", (data) => {
        console.log("doTempleTbl data: ", data);
    });
};

const doTotal = async () => {
    await $.get("http://liquorrus.hopto.org:3000/total", (data) => {
...
const sendOrder = async () => {
    ...
    for (p in urlItems) {
        await $.get(urlItems[p], function (data) {
            ...
            $("#itemizedBody").append(
                `<tr id = ${rowId}><td class = 'data'>${name}</td><td class = 'data'>$` +
                    price +
                    `</td></tr>`
            );
            counter++;
...
const doOutOfStockItems = async () => {
    //setTimeout is needed to ensure all the requests finish before the total is retrieved
    await $.get("http://liquorrus.hopto.org:3000/outofstock", (data) => {
        let o = JSON.stringify(data);
        let oItems = o.split(",");
...
const doInStockItems = async () => {
    //setTimeout is needed to ensure all the requests finish before the total is retrieved
    await $.get("http://liquorrus.hopto.org:3000/instock", function (data) {
        let o = JSON.stringify(data);
        let oItems = o.split(":");
        console.log(o);
    });
